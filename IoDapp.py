...
def inter_plot_HV(df, portee, scl=20, pep=None):
    ...
    layout = ((crvX.opts(xaxis='top')+hv.Empty()+(ds.to(hv.Image,['x','y']).opts(colorbar=True, cmap='gray')*sc)+crvY.opts(yaxis='right',invert_axes=True)).cols(2))
...
class DisplayLight(param.Parameterized):
    ...
    def view(self):
        ...
        if self.fig==None:
            ...
            return pn.Row((self.imgStade*self.bnd.opts(line_width=3, color='red')*stdPth).opts(height=500),self.fig)


dspl = DisplayLight()

iodApp = pn.Column(dspl.file_input, dspl.param.scl, dspl.view)
